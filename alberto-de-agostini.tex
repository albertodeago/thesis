...
\pagestyle{mypagestyle}
	...
	% set caption width to 80% instead of 100%
...
\begin{figure}[H]
	\centering
	\label{esempio-chat}
	\includegraphics[scale=0.75]{images/esempio-chat.jpg}
	\caption{THRON chat}
\end{figure}
...
Inoltre THRON usa una sistema di \gloss{CDN} esterne perchè gestire una \gloss{CDN} di proprietà grande ha un rapporto costi benefici molto alto. Grazie a questa rete riesce a operare in tutto il mondo, Cina compresa, come vediamo in \hyperref[thron-cdn]{Figura 1.4}
\begin{figure}[H]
	\centering
	\label{thron-cdn}
	\includegraphics[width=1.0\textwidth]{images/thron-cdn.jpg}
	\caption{THRON Content Delivery Network}
\end{figure}
...
\begin{figure}[h] %TODO controllare ad ogni build dove va a finire
	\centering
...
\end{itemize}
%TODO inserire figura workflow snowplow CUSTOM THRON
Tutti questi moduli sono software basati su Apache Spark scritti in scala. Leggono i dati da instanze di Elasticsearch popolate dai moduli precedenti e post elaborazione li salvano in altre istanze di Elasticsearch.\\
...
Di seguito troviamo delle immagini raffiguranti l'output che un cliente si ritrova. Le immagini sono state prese da degli ambienti di test THRON perciò con dati solamente per scopi di prova.\\
...
\end{figure}
In \hyperref[grafico-qualitativo-1]{Figura 3.5} possiamo vedere il grafico che mostra, per una scelta applicazione raccomandativa slave, una diagramma a barre con i valori delle impressions e clicks per vari contenuti. Alla destra abbiamo un riassunto in cui sono mostrate le impressions totali dell'applicazione, i clicks totali e due grafici a tag cloud in cui sono presenti nel primo i tag relativi la classe target dei contenuti, mentre in quello sottostante i tag relativi la classe topic dei contenuti. 
\begin{figure}[H]
	\centering
