...
\usepackage{graphicx}
\usepackage{wrapfig}
\usepackage{lastpage} % to have a reference to the last page of the book
\usepackage{etoolbox} % to rewrite the \chapter command, to add the same pagefooter style
\usepackage{caption} % for the image captions
\usepackage{enumitem}
\usepackage{emptypage} % to remove header and footer from empty pages
...
\cfoot{}
\rfoot{\thepage \hspace{1pt} di {\hypersetup{linkcolor = black}\pageref{LastPage}}}
...
\begin{figure}[h] %TODO controllare ad ogni build dove va
	\centering
...
\end{itemize} 
%TODO problemi prestazioni iniziali
